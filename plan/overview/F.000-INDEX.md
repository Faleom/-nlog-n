...
| F.006 | My World pipeline | 0 | P2 | P4 | Implemented, awaiting review | — |
| F.007 | Object → skill → steps lookup | 0 | P3 | P2 | Implemented, awaiting review | F.006 (shape only) |
| **F.008** | **Game 1, one level** ← **GATE** | **0** | **P2** | **P4** | Implemented, awaiting review | F.005, F.006, F.007 |
...
| F.014 | Branch 2: milestones + prompts | 1 | P3 | P2 | Implemented, awaiting review | F.001, F.002 |
| F.015 | Branch 2: card + guardrail | 1 | P3 | P2 | Implemented, awaiting review | F.014, F.008 |
| F.016 | Context profile: quick prefs | 2 | P4 | P1 | Implemented, awaiting review | F.001, F.005 |
| F.017 | Companion mechanic in Game 1 | 2 | P2 | P4 | Implemented, awaiting review | F.004, F.008, F.016 |
...
| F.020 | Branch handoff & no-screening | 2 | P3 | P2 | Implemented, awaiting review | F.010, F.015 |
| **F.021** | **Game 3: roadmap engine (Chapter 1 — Match the picture)**, redesigned from the original Shadow Match — see F.021.md | **2 — required** | **P2** | **P4** | Implemented, awaiting review | F.006, F.009, F.010 |
| **F.022** | **Game 2: Sequencing + routines** | **2 — required** | **P1** | **P3** | Implemented, awaiting review | F.008, F.009, F.016 |
...
| **F.027** | **Block-stack match** (Logic & Quantity track) — swipe up lifts / down places, replacing the − / + buttons | **2** | **P1** | **P3** | Implemented, awaiting review | F.001, F.010, F.013 |
| **F.028** | **Sort by rule** (Logic & Quantity track) — tap-then-tap primary, drag optional | **3 — stretch** | **P1** | **P3** | Implemented, awaiting review | F.001, F.010, F.013 |
...
## The Logic & Quantity track (F.027, F.028) — added after the original plan

A fourth track, specced in `Logic & Quantity Rounds.pdf`, of five wordless
rounds governed by one rule: **look at it** is in scope (everything needed
to answer stays on screen), **hold it** is out (remember a rule and apply it
later is executive function, not perception). Two of its five rounds are
built: **F.027 Block-stack match** and **F.028 Sort by rule**.

Unlike Branch 1's three games, these rounds use **abstract blocks and
shapes rather than the child's own room** — the same deliberate exception
Game 3 already takes, and the same thing to say plainly if it comes up
rather than letting it read as a contradiction of the app's own pitch.

**Two interaction decisions were made here and are recorded in the feature
files, not just the code:**
- **F.027 replaced a − / + button pair with a vertical swipe.** Those two
  glyphs were symbols inside a zero-text child view — a §7.10 violation,
  not a usability preference. Swipe **up lifts off**, **down places on**,
  held in one constant because the competing reading is its exact inverse.
  A tap does the same two actions by position, unadvertised.
- **F.028 kept tap-then-tap as the primary input**, with drag working but
  never required — the same call §8.2 and §8.3 already made, on the same
  fine-motor grounds.

**Still unbuilt from that track**, in its own ranked order: More or fewer +
What comes next (cheapest, share the most machinery), then Same amount.
Those three are ranked *ahead* of the two built here; F.027 was built first
because the track names it the best thing to show an audience.
