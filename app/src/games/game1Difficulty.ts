...
export function targetDescriptionForLevel(level: Game1Level, target: TaggedCrop): string {
  switch (level) {
    ...
    //
    // The GROUP is described by questPrompt() in games/game1/quests.ts —
    // that is the line the child actually hears at the start of a
    // collecting round. This function only names the one object in front
    // of them, which is why 3-5 collapse to a single plain case.
    case 3:
    case 4:
    case 5:
      return `your ${target.colour} ${target.name}`;
