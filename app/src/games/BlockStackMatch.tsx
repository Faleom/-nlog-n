...
import { motionFor, motionVars } from '../config/motion';
...
type Phase = 'idle' | 'building' | 'reportingSupport';
...
.bsm-floor {
  ...
  gap: clamp(28px, 8vw, 72px); min-height: 340px; padding: 8px 0;
}
...
export function BlockStackMatch({ profile, onChildFacingChange }: BlockStackMatchProps) {
  ...
  const motion = useMemo(() => motionFor(profile, prefersReduced), [profile, prefersReduced]);
