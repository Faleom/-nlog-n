...
export default defineConfig(({ mode, command }) => {
  // Vite only auto-populates import.meta.env for the client bundle — a
  // Node-side module loaded via ssrLoadModule (api/claude.ts) still reads
  // process.env directly, so app/.env's ANTHROPIC_API_KEY has to be copied
  // in by hand here for the dev proxy above to see it.
